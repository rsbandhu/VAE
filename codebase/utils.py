...
def sample_gaussian(m, v):
    ...
    z = m + torch.sqrt(v)*(torch.randn_like(m))
...
def log_normal(x, m, v):
    ...
    z_dim = m.shape[-1]
    #print("x,m,v shape &&&&  ", x.shape,m.shape, v.shape)
    x1 = -torch.sum(torch.log(v)/2.0 ,-1)
    x2 = - z_dim*torch.log(2.0*torch.tensor(np.pi))/2.0
    exponent = -torch.sum((x - m) * (x - m) / (2.0 * v), -1)

    log_prob = x1 + x2 + exponent
    #print("log prob shape ***  ", log_prob.shape)
    #print(log_prob[0], x1[0], x2, exponent[0])
    #print(log_prob[1], x1[1], x2, exponent[1])
...
def log_normal_mixture(z, m, v):
    ...
    (_, mix_size, latent_dim) = m.shape
    (batch_size, _ ) = z.shape

    #print("z,m,v shapes = ", z.shape, m.shape, v.shape)
    z_expanded = z.unsqueeze(1).expand(batch_size,mix_size,latent_dim)
    #print("z_expanded,m,v shapes = ", z_expanded.shape, m.shape, v.shape)
    exponent = - (z_expanded - m)*(z_expanded -m)/(2.0*v) - torch.log(v) #shape = (batch, mix, latent_dim)

    log_prob_temp = log_mean_exp(exponent, dim= 1) #shape = (batch, latent_dim)

    log_prob = torch.mean(log_prob_temp, -1) - latent_dim*torch.log(2.0*torch.tensor(np.pi))/2.0 #shape = batch, mix

    ln_temp = log_normal(z_expanded, m, v)
    #print("ln_temp shape = ", ln_temp.shape)
    log_prob = log_mean_exp(ln_temp, dim= 1)
...
def log_bernoulli_with_logits(x, logits):
    ...
    #print("log prob Bernoulli ", log_prob)
    return log_prob
