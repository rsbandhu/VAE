...
class SSVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        # my addition  *******
        log_p = torch.log(1.0/ 10.0)
        (m,v) = self.enc.encode(x, y=self.y_dim) #my addition
        z = ut.sample_gaussian(m, v)  # sample a point from the multivariate Gaussian
        y_x_logits = self.dec.decode(z)  # Calculate logits after passing the sampled "Z" through the decoder

        q_y_x = F.log_softmax(y_x_logits, dim=1) # calculate the softmax for each digit
        log_q_y_x = torch.log(q_y_x)
        KL_y = torch.mean(ut.kl_cat(q_y_x, log_q_y_x, log_p), -1) #Calculate KL_y
